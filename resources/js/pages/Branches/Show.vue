...
const buildPrintTitle = () => `Branch Profile - ${props.branch.name}`;
...
<template>
    <Head :title="`Branch - ${props.branch.name}`" />
    ...
    <AppLayout :breadcrumbs="props.breadcrumbs">
        <div class="space-y-6">
                ...
                <div class="liquidGlass-content flex flex-col gap-4 px-5 py-5 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-300">
                            Detailed view for {{ props.branch.name }}
                        </p>
                    </div>

                    <div class="flex flex-wrap items-center gap-2">
                        <GlassButton as="span" size="sm" variant="secondary">
                            <Link :href="route('branches.index')" class="flex items-center gap-2">
                        ...
                        <GlassButton as="span" size="sm" variant="primary">
                            <Link :href="route('branches.edit', props.branch.id)" class="flex items-center gap-2">
                                <Edit3 class="size-4" />
                                <span>Edit</span>
                            </Link>
                        </GlassButton>
            ...
            <div class="hidden print:block text-center text-slate-800">
                ...
                <p class="text-sm">Branch Profile: {{ props.branch.name }}</p>
                        ...
                        <div class="grid flex-1 gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <div class="space-y-2 rounded-lg border border-slate-200/70 bg-white/70 p-4 text-sm shadow-sm dark:border-slate-800/50 dark:bg-slate-900/60">
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Contact Person</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.branch.contact_person ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Email</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.branch.contact_email ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Phone</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.branch.contact_phone ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="space-y-2 rounded-lg border border-slate-200/70 bg-white/70 p-4 text-sm shadow-sm dark:border-slate-800/50 dark:bg-slate-900/60">
                                    <div>
                                        ...
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.branch.notes ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            >
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                        Audit trail for updates to this branch profile.
                    </p>
                </div>
