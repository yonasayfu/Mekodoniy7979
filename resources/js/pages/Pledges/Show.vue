...
const buildPrintTitle = () => `Pledge Details - ${props.pledge.user.name} to ${props.pledge.elder.first_name} ${props.pledge.elder.last_name}`;
...
<template>
    <Head :title="`Pledge - ${props.pledge.user.name} to ${props.pledge.elder.first_name} ${props.pledge.elder.last_name}`" />
    ...
    <AppLayout :breadcrumbs="props.breadcrumbs">
        <div class="space-y-6">
                ...
                <div class="liquidGlass-content flex flex-col gap-4 px-5 py-5 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-300">
                            Pledge by {{ props.pledge.user.name }} for {{ props.pledge.elder.first_name }} {{ props.pledge.elder.last_name }}
                        </p>
                    </div>

                    <div class="flex flex-wrap items-center gap-2">
                        <GlassButton as="span" size="sm" variant="secondary">
                            <Link :href="route('pledges.index')" class="flex items-center gap-2">
                        ...
                        <GlassButton as="span" size="sm" variant="primary">
                            <Link :href="route('pledges.edit', props.pledge.id)" class="flex items-center gap-2">
                                <Edit3 class="size-4" />
                                <span>Edit</span>
                            </Link>
                        </GlassButton>
            ...
            <div class="hidden print:block text-center text-slate-800">
                ...
                <p class="text-sm">Pledge Details: {{ props.pledge.user.name }} to {{ props.pledge.elder.first_name }} {{ props.pledge.elder.last_name }}</p>
                                ...
                                <div class="space-y-2 rounded-lg border border-slate-200/70 bg-white/70 p-4 text-sm shadow-sm dark:border-slate-800/50 dark:bg-slate-900/60">
                                    <div>
                                        ...
                                        <p class="font-medium text-slate-900 dark:text-slate-100">
                                            <Link :href="route('users.show', pledge.user.id)" class="text-indigo-600 hover:underline">
                                                {{ props.pledge.user.name }}
                                            </Link>
                                        </p>
                                    </div>
                                    <div>
                                        ...
                                        <p class="font-medium text-slate-900 dark:text-slate-100">
                                            <Link :href="route('elders.show', pledge.elder.id)" class="text-indigo-600 hover:underline">
                                                {{ props.pledge.elder.first_name }} {{ props.pledge.elder.last_name }}
                                            </Link>
                                        </p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Amount</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.pledge.amount ?? '-' }} ETB</p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Frequency</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.pledge.frequency ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Start Date</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.pledge.start_date ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">End Date</p>
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.pledge.end_date ?? '-' }}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="font-medium text-slate-900 dark:text-slate-100">{{ props.pledge.notes ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            >
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                        Audit trail for updates to this pledge.
                    </p>
                </div>
